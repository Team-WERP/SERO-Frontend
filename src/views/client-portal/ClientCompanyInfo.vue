<template>
    <div class="min-h-screen bg-gray-50 font-sans relative"> <header class="mb-8">
        <div>
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-40">
        ...
        <p class="mt-4 text-[#4C4CDD] font-medium">정보를 불러오는 중입니다...</p>
      </div>
  
      <div v-else-if="clientInfo" class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div class="lg:col-span-3 space-y-6">
          ...
          <section class="bg-white p-6 rounded-xl border border-gray-200 ">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 text-sm">
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">회사명</span>
                <span class="font-bold text-gray-900">{{ clientInfo.companyName }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">사업자번호</span>
                <span class="font-medium">{{ clientInfo.businessNumber }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">업태</span>
                <span class="font-medium">{{ clientInfo.businessType }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">종목</span>
                <span class="font-medium">{{ clientInfo.businessItem }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">대표자명</span>
                <span class="font-medium text-gray-800">{{ clientInfo.ceoName }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">대표 전화번호</span>
          ...
          <section class="bg-white p-6 rounded-xl border border-gray-200 ">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 text-sm">
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">담당자명</span>
                <span class="font-medium text-gray-900">{{ clientInfo.managerName }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2">
                <span class="w-32 text-gray-500 font-medium">연락처</span>
                <span class="font-medium">{{ clientInfo.managerContact }}</span>
              </div>
              <div class="flex border-b border-gray-100 pb-2 md:col-span-2">
                <span class="w-32 text-gray-500 font-medium">이메일 주소</span>
            ...
            <div class="bg-white p-6 rounded-2xl border-2 border-[#4C4CDD]/10 ">
              ...
              <div class="space-y-4 text-sm mb-6 border-b border-gray-100 pb-6">
                <div class="flex justify-between text-gray-500 font-medium">
                  <span>총 여신 한도</span>
                  <span class="text-gray-900 font-bold">₩ {{ formatNumber(clientInfo.creditLimit) }}</span>
                </div>
                <div class="flex justify-between text-gray-500 font-medium">
                  <span>현재 사용 미수금</span>
                  <span class="text-rose-600 font-bold">₩ {{ formatNumber(clientInfo.receivables) }}</span>
                </div>
              </div>
              <div class="flex flex-col gap-2">
                <span class="text-gray-500 font-semibold text-xs uppercase tracking-wider">가용 여신 잔액</span>
                <span class="text-[22px] font-semibold text-[#4C4CDD]">
              ...
              <div class="mt-6">
                <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                  <div 
                    class="bg-[#4C4CDD] h-full rounded-full transition-all duration-700" 
                    :style="{ width: calculateUsageRate(clientInfo) + '%' }"
                  ></div>
                </div>
                <p class="text-[11px] text-gray-400 mt-2 text-right font-medium">여신 한도 대비 {{ calculateUsageRate(clientInfo) }}% 사용 중</p>
              </div>
            </div>
  
            <div class="p-6 rounded-2xl border border-[#4C4CDD]/30 bg-[#4C4CDD]/5">
              <div class="space-y-4 text-sm">
                <div class="flex justify-between items-center">
                  <span class="text-gray-600 font-medium">영업팀 문의처</span>
                  <span class="font-bold font-mono text-[#4C4CDD]">02-1111-2222</span>
                </div>
              </div>
            </div>
          </div>
  ...
  const fetchDetail = async () => {
    try {
      ...
      const data = await getClientCompanyDetail(clientId)
