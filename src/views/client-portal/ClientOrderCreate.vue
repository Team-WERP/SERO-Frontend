<template>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div class="lg:col-span-3 space-y-8">
          <section class="bg-white p-6 rounded-xl border border-gray-200">
            <h2 class="text-lg font-bold mb-4 flex items-center gap-2">
              <span class="w-1 h-5 bg-indigo-600 rounded-full"></span> 주문 기본 정보
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label class="block text-[13px] font-medium text-gray-700 mb-1">납기 요청일 <span class="text-red-500">*</span></label>
                <input v-model="form.requestDate" type="date" class="w-full border rounded-lg p-2.5 text-[13px] border-gray-400 focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
            </div>
          </section>
          ...
          <section class="bg-white p-6 rounded-xl border border-gray-200">
            <div class="flex justify-between items-center mb-4">
              <h2 class="text-lg font-bold flex items-center gap-2">
                <span class="w-1 h-5 bg-indigo-600 rounded-full"></span> 주문 품목
              </h2>
            </div>
            ...
            <div class="flex gap-2">
              <button @click="isItemModalOpen = true" class="bg-indigo-600 text-white px-5 py-2 rounded-lg font-bold text-[15px] hover:bg-indigo-700">품목 추가</button>
              <button class="border border-indigo-600 text-indigo-600 px-5 py-2 rounded-lg font-bold hover:bg-indigo-50">납기 가능 여부 조회</button>
            </div>
          </section>
  
          <section class="bg-white p-6 rounded-xl border border-gray-200">
            <h2 class="text-lg font-bold mb-4 flex items-center gap-2">
              <span class="w-1 h-5 bg-indigo-600 rounded-full"></span> 납품/배송 정보
            </h2>
            <div class="space-y-4">
              <div class="flex items-center gap-4">
                <label class="w-32 text-[13px] font-bold text-gray-700">납품지 <span class="text-red-500">*</span></label>
                <select v-model="selectedAddrId" @change="onAddressChange" class="flex-1 border text-[13px] rounded-lg p-2.5 border-gray-400 outline-none">
        ...
        <div class="lg:col-span-1">
          <div class="sticky top-8 space-y-4">
            <div class="bg-white p-6 rounded-2xl border border-gray-200">
              ...
              <div class="mb-8">
                <div class="flex justify-between items-end font-semibold">
                  ...
                  <span class="text-lg font-bold text-indigo-600">₩ {{ formatNumber(totalAmount) }}</span>
                </div>
              </div>
  
              <div class="space-y-4 border-t pt-6 text-[13px]">
                <div>
                  <p class="text-[12px] text-gray-400 mb-1 font-medium">납품처 (Ship-to)</p>
                  <p class="font-semibold text-gray-800">{{ currentAddressName || '-' }}</p>
                </div>
                <div>
            ...
            <button 
              @click="handleSubmit" 
              class="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold text-[15px] hover:bg-indigo-700 shadow-md transition-all active:scale-[0.98]"
            >
              주문서 제출하기
            </button>
          </div>
  ...
  import { createClientOrder } from '@/api/order/salesOrder';
  import ItemSearchModal from '@/components/client/ItemSearchModal.vue';
  ...
  const handleSubmit = async () => {
    ...
    const orderPayload = {
      shippedAt: `${form.value.requestDate} 16:00`,
