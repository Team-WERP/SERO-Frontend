  ...
  <div v-else-if="order" class="min-h-screen bg-[#F9FAFB] p-6 font-sans">
    ...
    <div class="rounded-xl bg-white p-6 border border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <div class="rounded-xl border border-gray-200 p-5 bg-white min-h-[220px]">
            ...
            <div class="space-y-3 text-sm">
              <div class="flex justify-between"><span class="text-gray-500 font-bold">주문번호</span><span
                  class="text-black">{{ order.orderCode }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 font-bold">주문일</span><span
                  class="text-black">{{ order.orderedAt }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 font-bold">납기일</span><span
                  class="text-black">{{ order.shippedAt }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 font-bold">비고</span><span
                  class="max-w-[150px] text-right text-black">{{ order.note || '-' }}</span></div>
            </div>
          </div>
          ...
          <div class="rounded-xl border border-gray-200 p-5 bg-white min-h-[220px]">
            ...
            <div class="space-y-3 text-sm">
              <div class="flex justify-between"><span class="text-gray-500 font-bold">고객사명</span><span
                  class="text-black">{{ order.clientName }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 font-bold">담당자명</span><span
                  class="text-black">{{ order.clientManagerName }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 font-bold">연락처</span><span
                  class="text-black">{{ order.clientManagerContact }}</span></div>
            </div>
          </div>
          ...
          <div class="rounded-xl border border-gray-200 p-5 bg-white min-h-[220px]">
            <div class="flex justify-between items-center mb-4">
              ...
              <span
                class="rounded bg-green-50 px-2 py-0.5 text-[10px] font-bold text-green-600 border border-green-200">✓
                주문 가능</span>
            </div>
            <div class="space-y-3 text-sm">
              <div class="flex justify-between"><span class="text-gray-500 ">여신 한도</span><span class="text-black">₩ {{
                formatPrice(order.creditLimit) }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 ">주문 가능 금액</span><span class="text-black">₩
                  {{ formatPrice(order.availablePrice) }}</span></div>
              <div class="flex justify-between"><span class="text-gray-500 ">이번 주문 금액</span><span
                  class="text-black font-bold">₩ {{ formatPrice(order.totalPrice) }}</span></div>
              <div class="flex justify-between border-t pt-2 mt-2"><span class="text-gray-500">주문 반영 후 가능 금액</span><span
                  class="text-green-600 font-bold">₩ {{ formatPrice(order.remainingAmount) }}</span></div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-8">
          ...
          <div class="relative rounded-xl border border-gray-200 bg-white min-h-[300px] overflow-hidden">
            ...
            <template v-if="approvalData">
              ...
              <div class="overflow-x-auto m-6">
                ...
                <div class="flex justify-end mt-4">
                  <router-link v-if="!['ORD_RED', 'ORD_RVW'].includes(order.status)"
            ...
            <div v-else-if="!isApprovalLoading" class="flex flex-col items-center justify-center py-16 px-4">
              ...
              <button v-if="order.status === 'ORD_RVW' && order.managerId == userStore.user?.id"
        ...
        <div class="mb-8">
                ...
                <tr v-for="(hItem, idx) in itemHistory?.items" :key="hItem.itemId" class="text-center hover:bg-gray-50">
                  ...
                  <td class="px-2 py-4">
                    <button @click="openHistoryModal(hItem.itemId, hItem.item.itemName)"
                      class="text-[13px] underline hover:text-gray-700">이력 조회</button>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div class="flex gap-2 justify-end mb-5">
            <template v-if="canCreateDeliveryOrProduction">
              <button @click="openPRModal"
                class="rounded-lg bg-[#4C4CDD] px-4 py-2 text-sm font-bold text-white hover:bg-[#3b3bbb]">생산 설정</button>
              <button @click="openDOModal"
                class="rounded-lg bg-[#4C4CDD] px-4 py-2 text-sm font-bold text-white hover:bg-[#3b3bbb]">납품 설정</button>
            </template>
          </div>

          <ProductionRequestModal v-if="isPRModalOpen" :soId="Number(route.params.orderId)" :items="order.items"
            :isUpdate="isPRUpdateMode" :initialData="prInitialData" @close="isPRModalOpen = false"
            @submit="handlePRDraftSubmit" />

          <DeliveryOrderModal v-if="isDOModalOpen" :soId="Number(route.params.orderId)" :items="order.items"
            :historyItems="itemHistory?.items" @close="isDOModalOpen = false" @submit="handleDOSubmit" />
              ...
              <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
                <table class="w-full text-sm text-center table-fixed">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(doc, dIdx) in section.data" :key="dIdx"
                      ...
                      <td class="py-4">
                        <span :class="section.badgeFn(doc.status)"
                      ...
                      <td class="py-4 text-center">
                        <button
                          ...
                          @click="openDocument(doc)" class="text-gray-400 hover:text-[#4C4CDD] transition-all"
                          title="문서 보기">
                      ...
                      <td class="py-4 text-center">
                        <div class="flex justify-center items-center gap-1.5 px-2">
                          <button v-if="section.title === '생산 요청 문서' && doc.status === 'PR_TMP'"
                            ...
                            class="rounded border border-gray-300 px-2 py-1 text-[11px] font-bold text-gray-600 hover:bg-gray-50 whitespace-nowrap">수정</button>
                          <router-link :to="getLinkPath(section.type, doc)"
                            class="h-7 flex items-center justify-center rounded bg-[#4C4CDD] px-2.5 text-[11px] font-bold text-white hover:bg-[#3b3bbb] whitespace-nowrap">바로가기</router-link>
                        </div>
                      </td>
                    </tr>
                  </tbody>
      ...
      <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
        ...
        <CancelOrderModal v-if="isCancelModalOpen" :is-submitting="isCancelling" @close="isCancelModalOpen = false"
          @confirm="handleCancelConfirm" />
        <button v-if="order && order.status === 'ORD_RED'" @click="openAssignmentModal"
      ...
      <ManagerAssignmentModal v-if="isModalOpen" :departmentData="deptEmployees" @close="isModalOpen = false"
    ...
    <div v-if="isHistoryModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div class="w-full max-w-4xl rounded-2xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b p-5">
          <h3 class="text-xl font-bold text-gray-900"><span class="text-[#4C4CDD]">{{ selectedItemName }}</span> 변동 이력
          </h3>
                  ...
                  <tr v-if="historyDetails.length === 0">
                    <td colspan="8" class="py-12">
                      <div class="flex flex-col items-center justify-center">
                        <img src="@/assets/새로이새로미.png" alt="No Data" class="mb-4 h-16 w-auto opacity-40" />
        ...
        <div class="flex justify-end p-5 border-t">
          <button @click="isHistoryModalOpen = false"
            class="rounded-lg bg-gray-100 px-6 py-2 text-sm font-bold text-gray-600 hover:bg-gray-200">닫기</button>
        </div>
