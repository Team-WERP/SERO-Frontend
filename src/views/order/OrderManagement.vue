<template>
    <div class="p-8 bg-gray-50 min-h-screen font-sans">
      <header class="mb-8">
        ...
        <p class="text-sm text-gray-500 mt-1">전체 주문을 상태별로 필터링하여 조회합니다.</p>
      </header>
      ...
      <div class="flex gap-2 mb-8">
        <button 
          ...
          :class="[
            'px-6 py-2 rounded-full border text-sm font-semibold transition-all shadow-sm',
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-7 gap-4 items-end">
          ...
          <div>
            ...
            >
              ...
              <option value="ORD_RED">접수대기</option>
              <option value="ORD_RVW">주문검토</option>
              <option value="ORD_APPR_PEND">주문결재중</option>
              <option value="ORD_APPR_DONE">결재승인</option>
              <option value="ORD_APPR_RJCT">결재반려</option>
              <option value="ORD_WORK_REQ">작업요청</option>
              <option value="ORD_PRO">생산중</option>
              <option value="ORD_SHIP_READY">출고중</option>
              <option value="ORD_SHIPPING">배송중</option>
              <option value="ORD_DONE">배송완료</option>
              <option value="ORD_CANCEL">주문취소</option>
            </select>
          </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            <thead>
              <tr class="bg-gray-50 text-gray-500 text-[13px] font-bold uppercase border-b border-gray-200 text-center">
                ...
                <th class="px-5 py-3">주문일</th>
                <th class="px-5 py-3">납기일</th>
                <th class="px-5 py-3">담당자</th>
                <th class="px-5 py-3">진행상태</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(order, index) in orders" :key="order.orderId" class="hover:bg-indigo-50/30 transition-colors text-sm text-black cursor-pointer text-center">
                <td class="px-5 py-4 text-gray-400 text-xs">{{ (currentPage - 1) * pageSize + index + 1 }}</td>
                <td class="px-5 py-4 font-semibold text-gray-900">{{ order.soCode }}</td>
                <td class="px-5 py-4">{{ order.clientName }}</td>
                <td class="px-5 py-4">
                  {{ order.mainItemName }}
                  <span v-if="order.totalItemCount > 1" class="ml-1">외 {{ order.totalItemCount - 1 }}건</span>
                </td>
              ...
              <tr v-if="orders.length === 0">
                <td colspan="9" class="px-5 py-24 text-center text-gray-400">조회된 데이터가 없습니다.</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="px-5 py-4 border-t border-gray-100 flex items-center justify-center gap-4 bg-gray-50/30">
          <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1" class="px-4 py-1.5 border border-gray-300 rounded-md text-sm font-semibold bg-white text-gray-600 disabled:opacity-30 hover:bg-gray-50">이전</button>
          <span class="text-sm font-bold text-gray-700">{{ currentPage }}</span>
          <button @click="changePage(currentPage + 1)" :disabled="isLastPage" class="px-4 py-1.5 border border-gray-300 rounded-md text-sm font-semibold bg-white text-gray-600 disabled:opacity-30 hover:bg-gray-50">다음</button>
        </div>
      </div>
    </div>
  </template>
  ...
  import { getSOList } from '@/api/order/orderRequest';
